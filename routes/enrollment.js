...
router.post('/submit', async (req, res) => {
    try {
        ...
        if (existingTx) return res.status(400).json({ message: "This Transaction ID has already been used." });

        const newRequest = new Enrollment({ name, email, phone, transactionId, paymentMethod });
        await newRequest.save();

        res.status(200).json({ message: "Enrollment request submitted! Waiting for Commander approval." });
    } catch (err) { res.status(500).send('Server Error'); }
});
...
router.get('/pending', [authMiddleware, verifyAdmin], async (req, res) => {
    try {
        const requests = await Enrollment.find({ status: 'pending' }).sort({ createdAt: -1 });
        res.json(requests);
    } catch (err) { res.status(500).send('Server Error'); }
});
